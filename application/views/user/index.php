 <script>
  $(function(){
    ...
    $("#btn-confirm-yes").click(function(){
        ...
        $("#delModal").modal('hide');  
        window.location.replace("<?php echo base_url().'user/delete/'; ?>"+del_id);
    });
        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <!-- search form -->
                    <div class="row">
                        <form role="form" action="<?=base_url()?>user/index">
                            <div class="col-lg-2">
                                <div class="form-group">
                                    <input class="form-control input-sm" name="email" placeholder="Email" value="<?=isset($_GET['email'])?$_GET['email']:''?>">
                                </div>
                            </div>
                            <div class="col-lg-2">
                        ...
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <?php foreach ($users as $cur): ?>
                                <tr>
                                    <td><?=$cur->email?></td>
                                    <td><?=$cur->act_code?></td>
                                    <td><?=($cur->is_activate == 1 ? 'Yes' : 'No')?></td>
                                    <td><?=($cur->is_facebook == 1 ? 'Yes' : 'No')?></td>
                                    <td><?=date('d M Y',$cur->date)?></td>
                                    <td>
                                        <a href="<?=base_url()?>user/edit/<?=$cur->user_id?>"><i class="fa fa-pencil-square-o" data-toggle="tooltip" data-placement="left" data-original-title="Edit"></i></a>&nbsp
                                        <a href="<?=base_url()?>user/delete/<?=$cur->user_id?>" class="delete-link"><i class="fa fa-trash-o" data-toggle="tooltip" data-placement="left" data-original-title="Delete"></i></a>
                                    </td>
                                </tr>
                            <?php endforeach ?>
                            <?php endif ?>
